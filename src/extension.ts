...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Register Tree View
    const treeView = vscode.window.createTreeView('docsExplorer', {
    ...
    docsTreeProvider.onDocsFound(() => {
        vscode.commands.executeCommand('setContext', 'copilot-missingmanual.docsFound', true);
    });
    ...
    setTimeout(() => {
        if (!docsTreeProvider.hasDocsPath()) {
            ...
            vscode.commands.executeCommand('setContext', 'copilot-missingmanual.docsFound', false);
        }
    }, 5000);

    // Register commands
    const refreshCommand = vscode.commands.registerCommand('copilot-missingmanual.refreshDocs', () => {
        docsTreeProvider.refresh();
        vscode.window.showInformationMessage('Documentation refreshed.');
    });


    const openDocsCommand = vscode.commands.registerCommand('copilot-missingmanual.openDocs', (docItem) => {
        docsContentProvider.openDocument(docItem);
    });

    const searchDocsCommand = vscode.commands.registerCommand('copilot-missingmanual.searchDocs', async () => {
        ...
        const contentResults = await docsSearchProvider.searchContent(query);
        ...
        if (selected) {
            ...
            if (startIdx !== -1) {
                ...
                const highlightEnd = new vscode.Position(selected.line, startIdx + query.length);
                editor.selection = new vscode.Selection(highlightStart, highlightEnd);
            }
        }
    });

    const insertIntoCopilotCommand = vscode.commands.registerCommand('copilot-missingmanual.insertIntoCopilot', async (docItem) => {
    ...
    const docsWatcher = vscode.workspace.createFileSystemWatcher('**/docs/**/*.md');
